(function(c) {
    ...
    $.validator.setDefaults({
        ...
        errorPlacement: function (error, element){
            error.addClass('tips-error');
            var placement = element.last('.controls').siblings('.icon-error');
            placement.siblings('.tips-error').remove();
            error.insertAfter(placement);
        },
        highlight: function(element, errorClass, validClass) {
            var $ele = $(element);
            $ele.last('.controls').removeClass(validClass).addClass(errorClass);
            var placement = $ele.last('.controls');
            if(!placement.siblings('.icon-error').length) {
                $('<i class="icon-error"></i>').insertAfter(placement);
            }
        },
        unhighlight: function( element, errorClass, validClass){
            var $ele = $(element);
            if('select-one' === element.type){
                $ele.last('.controls').siblings('.tips-error').remove();
            }
            $ele.last('.controls').removeClass(errorClass).addClass(validClass).last('.controls').siblings('.icon-error').remove();
        }
    });
